...
export class AboutView extends EmraldWorkspaceView {
	...
	async onOpen() {
		...
		for (const s of specItems) {
			...
			setIcon(chip.createEl('span', { cls: 'emerald-wv-about-speckchip-icon' }), s.icon);
		...
		this.makeSection(container, 'what-it-isnt', 'x-circle', 'What EMRALD Is Not', el => {
			...
			this.bullet(list as HTMLElement, 'A checklist or to-do tracker — It doesn\'t tell you what to do');
			this.bullet(list as HTMLElement, 'A calendar or time-blocking app — It doesn\'t schedule your day');
			this.bullet(list as HTMLElement, 'A deliverables manager — It doesn\'t track whether things got done');
			this.bullet(list as HTMLElement, 'A hustle optimizer — It won\'t push you to do more with less');
			this.bullet(list as HTMLElement, 'A replacement for your brain — It works with your existing projects in Obsidian');
		...
		this.makeSection(container, 'what-you-bring', 'play-circle', 'What You Bring', el => {
			...
			for (const s of steps) this.bullet(list as HTMLElement, s);
		...
		this.makeSection(container, 'what-you-get', 'gift', 'What EMRALD Gives Back', el => {
			...
			const list = el.createEl('ul', { cls: 'emerald-wv-about-bullet-list' });
			this.bullet(list as HTMLElement, 'D-Metrics (D1–D20): Twenty diagnostic measurements of your effort patterns, energy balance, and work rhythm');
			this.bullet(list as HTMLElement, 'AI Insight Logs: Observations, suggestions, and discoveries about your patterns across 5 categories');
			this.bullet(list as HTMLElement, "Burnout Monitoring: Early warnings when your effort distribution suggests you\'re heading toward exhaustion");
			this.bullet(list as HTMLElement, 'Effort Digests: Weekly and monthly summaries of where your energy went');
			this.bullet(list as HTMLElement, 'Completion Rate: Clarity on which projects you finish versus abandon');
			this.bullet(list as HTMLElement, 'Calibration Over Time: EMRALD adjusts its model of you the more data it has, getting smarter and more accurate');
			this.bullet(list as HTMLElement, "75+ data points feed into every metric: EMRALD processes your sessions, energy, and calibration data across 20 D-metrics and layered internal measurements to surface patterns you'd never catch on your own.");
		...
		this.makeSection(container, 'guardrails', 'shield', 'Why the Guardrails Exist', el => {
			...
			const list = el.createEl('ul', { cls: 'emerald-wv-about-bullet-list' });
			this.bullet(list as HTMLElement, "Your day has a finite capacity, whatever hours you've set in your schedule. EMRALD won't let you pretend you have more.");
			this.bullet(list as HTMLElement, 'E4 work is genuinely exhausting. You can only do so much of it before the quality of everything drops.');
			this.bullet(list as HTMLElement, "Filling your day to 100% with demanding work is a fast path to burnout — EMRALD's allocation system is designed to prevent exactly that.");
			this.bullet(list as HTMLElement, "Early data will look incomplete. Charts won\'t fill in nicely for the first couple weeks. This is normal — the system is learning your baseline before it can tell you meaningful things about deviations.");
		...
		this.makeSection(container, 'core-systems', 'layers', 'Core Systems', el => {
			...
			const list = el.createEl('ul', { cls: 'emerald-wv-about-bullet-list' });
			this.bullet(list as HTMLElement, 'E-Levels (E1–E4): Four effort tiers from light to maximum. You assign them to projects to set expectations for how much energy a session will cost.');
			this.bullet(list as HTMLElement, 'D-Metrics (D1–D20): Twenty computed measurements of your effort patterns. D1–D8 are available to all users; D9–D20 unlock with Pro.');
			this.bullet(list as HTMLElement, "Burnout Monitor: Tracks your effort distribution across E-levels and warns you when patterns suggest you\'re overextended.");
			this.bullet(list as HTMLElement, 'Data Center: The visual home of all your D-metrics, with charts and context for each measurement.');
			this.bullet(list as HTMLElement, "Insight Log: AI-generated observations, suggestions, and discoveries about your effort patterns.");
			this.bullet(list as HTMLElement, 'Daily Check-in & Effort Receipt: The two feedback inputs that power the entire system.');
		...
		this.makeSection(container, 'who-its-for', 'users', 'Who EMRALD Helps', el => {
			...
			const list = el.createEl('ul', { cls: 'emerald-wv-about-bullet-list' });
			this.bullet(list as HTMLElement, 'Polymath Operators — people who run multiple creative or professional projects simultaneously and need to understand how their energy divides across them');
			this.bullet(list as HTMLElement, "Neurodivergent Individuals — people who experience energy and focus differently and need a system that adapts to them, not the other way around");
			this.bullet(list as HTMLElement, 'Students — people managing coursework, side projects, and exam prep across the same finite week');
			this.bullet(list as HTMLElement, "Creators — writers, artists, builders who work on long-horizon projects that don't fit into a task-tracker");
			this.bullet(list as HTMLElement, "Knowledge Workers — people whose output isn't easily measured by hours logged or tasks checked off");
			this.bullet(list as HTMLElement, 'Anyone Juggling Multiple Identities — parent, professional, hobbyist, student — all in the same person, all with legitimate energy claims on your day');
		...
		this.makeSection(container, 'learn-more', 'book-open', 'Learn More', el => {
			...
			this.link(linkWrap as HTMLElement, 'https://effortmastery.com', 'effortmastery.com — deeper reading on effort management');
			this.link(linkWrap as HTMLElement, 'https://app.effortmastery.com', 'app.effortmastery.com — manage your EMRALD account');
		});
